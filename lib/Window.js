...
module.exports = (Item, registry) => {
  return class Window extends EventEmitter {
    ...
    findItemsRange (start, end, itemType, metadata, notFull, nbt, withoutCraftResultSlot = false, durability) {
      const items = []
      while (start < end) {
        const item = this.findItemRange(start, end, itemType, metadata, notFull, nbt, withoutCraftResultSlot, durability)
        if (!item) break
        start = item.slot + 1
        items.push(item)
      }
    ...
    findItemRange (start, end, itemType, metadata, notFull, nbt, withoutCraftResultSlot = false, durability) {
      ...
      for (let i = start; i < end; ++i) {
        ...
        if (
          ...
          (durability == null || durability > item.nbt.value.Damage.value) &&
    ...
    findItemRangeName (start, end, itemName, metadata, notFull, durability) {
      assert.notStrictEqual(itemName, null)
      for (let i = start; i < end; ++i) {
        const item = this.slots[i]
        if (item && itemName === item.name &&
          (metadata == null || metadata === item.metadata) &&
          (durability == null || durability > item.durability) &&
          (!notFull || item.count < item.stackSize)) {
          return item
        }
      }
    ...
    findInventoryItem (item, metadata, notFull, durability) {
      assert(typeof item === 'number' || typeof item === 'string' || typeof item === 'undefined', 'No valid type given')
      return typeof item === 'number'
        ? this.findItemRange(this.inventoryStart, this.inventoryEnd, item, metadata, notFull, null, null, durability)
        : this.findItemRangeName(this.inventoryStart, this.inventoryEnd, item, metadata, notFull, null, null, durability)
    }

    findContainerItem (item, metadata, notFull, durability) {
      assert(typeof item === 'number' || typeof item === 'string' || typeof item === 'undefined', 'No valid type given')
      return typeof item === 'number'
        ? this.findItemRange(0, this.inventoryStart, item, metadata, notFull, null, null, durability)
        : this.findItemRangeName(0, this.inventoryStart, item, metadata, notFull, null, null, durability)
    }
